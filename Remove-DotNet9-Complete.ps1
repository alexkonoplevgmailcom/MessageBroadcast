...
if (Test-Path $uninstallTool) {
    ...
    try {
        ...
        if ($process.ExitCode -eq 0) {
            Write-Host "[SUCCESS] SDK removal completed successfully" -ForegroundColor Green
        } else {
            Write-Host "[ERROR] SDK removal failed with exit code: $($process.ExitCode)" -ForegroundColor Red
        }
    }
    catch {
        Write-Host "[ERROR] SDK removal error: $_" -ForegroundColor Red
    }
} else {
    ...
    foreach ($product in $dotnet9Products) {
        try {
            ...
            if ($result.ReturnValue -eq 0) {
                ...
                Write-Host " [ERROR] (Code: $($result.ReturnValue))" -ForegroundColor Red
            }
        }
        catch {
...
try {
    ...
    if ($sdks) {
        ...
        if ($net9Sdks) {
            Write-Host "WARNING: .NET 9 SDKs still found!" -ForegroundColor Red
        } else {
            Write-Host "[SUCCESS] No .NET 9 SDKs found" -ForegroundColor Green
        }
    } else {
...
try {
    ...
    if ($runtimes) {
        ...
        if ($net9Runtimes) {
            Write-Host "WARNING: .NET 9 Runtimes still found!" -ForegroundColor Red
        } else {
            Write-Host "[SUCCESS] No .NET 9 Runtimes found" -ForegroundColor Green
        }
    } else {
...
if ($remainingNet9.Count -gt 0) {
    Write-Host ""
    Write-Host "WARNING: Some .NET 9 components may still remain:" -ForegroundColor Red
    ...
    Write-Host "[SUCCESS] All .NET 9 components appear to be removed successfully!" -ForegroundColor Green
}
